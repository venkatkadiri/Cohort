...
Elasticsearch-powered query and search indexing service across users, bookings, and lectures.

---

## 📌 Service Specifications
- **Package Location**: `hubs/search-hub`
- **HTTP Port**: `:8002`
- **gRPC Port**: `:50053`
...
pnpm --filter @cohort/search-hub dev

# Typecheck TypeScript code
pnpm --filter @cohort/search-hub typecheck

# Build production bundle
pnpm --filter @cohort/search-hub build
```

### 2. Docker Containerization
A multi-stage, rootless production Dockerfile is maintained at `hubs/search-hub/Dockerfile`:
```bash
# Build Docker image
docker build -f hubs/search-hub/Dockerfile -t search-hub:latest .

# Run container locally
docker run -p 8002:8002 search-hub:latest
```
...
- Feature branches: `feature/search-hub/<short-description>` (e.g. `feature/search-hub/add-filter-options`)
- Bugfix branches: `fix/search-hub/<short-description>` (e.g. `fix/search-hub/handle-null-pointer`)
- Chore branches: `chore/search-hub/<short-description>`
...
- **Scope**: Must match this package name: `(search-hub)`
- **Examples**:
  - `feat(search-hub): add webhook notification dispatcher`
  - `fix(search-hub): correct database connection retry logic`
  - `refactor(search-hub): migrate configuration parser to strict schema`

### 3. Local PR Title Validation
Verify your proposed PR title locally before submitting:
```bash
pnpm lint:pr "feat(search-hub): support real-time updates"
```
...
The cloud infrastructure for this package is self-contained under `hubs/search-hub/terraform/`.
...
./cohort-tools.sh tf search-hub validate
# or validate across all packages:
./cohort-tools.sh tf:validate:all

# Run Terraform plan (dry-run)
./cohort-tools.sh tf search-hub plan
```
...
Package-level Helm charts are maintained at `hubs/search-hub/helm/`:
...
./scripts/minikube.sh build-images search-hub
...
The package includes dedicated pipelines under `hubs/search-hub/.github/`:
1. **`workflows/releaseApplication.yaml`**:
   - Builds `hubs/search-hub/Dockerfile`.
   - Pushes image to `ghcr.io/${{ github.repository_owner }}/search-hub` (without JFrog).
   - Tags image with commit SHA, `:latest`, and semantic version.
2. **`workflows/deployApplication.yaml`**:
   - Deploys `hubs/search-hub/helm` chart across environments (`override_envs`) and regions (`override_regions`).
3. **Renovate & Release-Please Chore PRs**:
   - Creates chore PRs titled `chore(search-hub): release`.
   - Automatically bumps `image.tag` in `hubs/search-hub/helm/values.yaml` and version in `Chart.yaml`.
