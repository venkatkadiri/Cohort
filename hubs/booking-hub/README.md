...
Booking, scheduling, and calendar slot reservation engine integrated with Temporal workflow client.

---

## 📌 Service Specifications
- **Package Location**: `hubs/booking-hub`
- **HTTP Port**: `:8004`
- **gRPC Port**: `:50051`
...
pnpm --filter @cohort/booking-hub dev

# Typecheck TypeScript code
pnpm --filter @cohort/booking-hub typecheck

# Build production bundle
pnpm --filter @cohort/booking-hub build
```

### 2. Docker Containerization
A multi-stage, rootless production Dockerfile is maintained at `hubs/booking-hub/Dockerfile`:
```bash
# Build Docker image
docker build -f hubs/booking-hub/Dockerfile -t booking-hub:latest .

# Run container locally
docker run -p 8004:8004 booking-hub:latest
```
...
- Feature branches: `feature/booking-hub/<short-description>` (e.g. `feature/booking-hub/add-filter-options`)
- Bugfix branches: `fix/booking-hub/<short-description>` (e.g. `fix/booking-hub/handle-null-pointer`)
- Chore branches: `chore/booking-hub/<short-description>`
...
- **Scope**: Must match this package name: `(booking-hub)`
- **Examples**:
  - `feat(booking-hub): add webhook notification dispatcher`
  - `fix(booking-hub): correct database connection retry logic`
  - `refactor(booking-hub): migrate configuration parser to strict schema`

### 3. Local PR Title Validation
Verify your proposed PR title locally before submitting:
```bash
pnpm lint:pr "feat(booking-hub): support real-time updates"
```
...
The cloud infrastructure for this package is self-contained under `hubs/booking-hub/terraform/`.
...
./cohort-tools.sh tf booking-hub validate
# or validate across all packages:
./cohort-tools.sh tf:validate:all

# Run Terraform plan (dry-run)
./cohort-tools.sh tf booking-hub plan
```
...
Package-level Helm charts are maintained at `hubs/booking-hub/helm/`:
...
./scripts/minikube.sh build-images booking-hub
...
The package includes dedicated pipelines under `hubs/booking-hub/.github/`:
1. **`workflows/releaseApplication.yaml`**:
   - Builds `hubs/booking-hub/Dockerfile`.
   - Pushes image to `ghcr.io/${{ github.repository_owner }}/booking-hub` (without JFrog).
   - Tags image with commit SHA, `:latest`, and semantic version.
2. **`workflows/deployApplication.yaml`**:
   - Deploys `hubs/booking-hub/helm` chart across environments (`override_envs`) and regions (`override_regions`).
3. **Renovate & Release-Please Chore PRs**:
   - Creates chore PRs titled `chore(booking-hub): release`.
   - Automatically bumps `image.tag` in `hubs/booking-hub/helm/values.yaml` and version in `Chart.yaml`.
