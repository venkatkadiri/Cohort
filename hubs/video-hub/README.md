...
- **Package Location**: `hubs/video-hub`
- **HTTP Port**: `:8005`
- **gRPC Port**: `:50055`
...
A multi-stage, rootless production Dockerfile is maintained at `hubs/video-hub/Dockerfile`:
```bash
# Build Docker image
docker build -f hubs/video-hub/Dockerfile -t video-hub:latest .
...
The cloud infrastructure for this package is self-contained under `hubs/video-hub/terraform/`.
...
Package-level Helm charts are maintained at `hubs/video-hub/helm/`:
...
The package includes dedicated pipelines under `hubs/video-hub/.github/`:
1. **`workflows/releaseApplication.yaml`**:
   - Builds `hubs/video-hub/Dockerfile`.
...
2. **`workflows/deployApplication.yaml`**:
   - Deploys `hubs/video-hub/helm` chart across environments (`override_envs`) and regions (`override_regions`).
3. **Renovate & Release-Please Chore PRs**:
   - Creates chore PRs titled `chore(video-hub): release`.
   - Automatically bumps `image.tag` in `hubs/video-hub/helm/values.yaml` and version in `Chart.yaml`.
