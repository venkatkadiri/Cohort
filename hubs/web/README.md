...
A multi-stage, rootless production Dockerfile is maintained at `hubs/web/Dockerfile`:
```bash
# Build Docker image
docker build -f hubs/web/Dockerfile -t web:latest .
...
The cloud infrastructure for this package is self-contained under `hubs/web/terraform/`.
...
Package-level Helm charts are maintained at `hubs/web/helm/`:
...
The package includes dedicated pipelines under `hubs/web/.github/`:
1. **`workflows/releaseApplication.yaml`**:
   - Builds `hubs/web/Dockerfile`.
...
2. **`workflows/deployApplication.yaml`**:
   - Deploys `hubs/web/helm` chart across environments (`override_envs`) and regions (`override_regions`).
3. **Renovate & Release-Please Chore PRs**:
   - Creates chore PRs titled `chore(web): release`.
   - Automatically bumps `image.tag` in `hubs/web/helm/values.yaml` and version in `Chart.yaml`.
