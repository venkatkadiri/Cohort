...
- **Package Location**: `hubs/transaction-hub`
- **HTTP Port**: `:8008`
...
A multi-stage, rootless production Dockerfile is maintained at `hubs/transaction-hub/Dockerfile`:
```bash
# Build Docker image
docker build -f hubs/transaction-hub/Dockerfile -t transaction-hub:latest .
...
The cloud infrastructure for this package is self-contained under `hubs/transaction-hub/terraform/`.
...
Package-level Helm charts are maintained at `hubs/transaction-hub/helm/`:
...
The package includes dedicated pipelines under `hubs/transaction-hub/.github/`:
1. **`workflows/releaseApplication.yaml`**:
   - Builds `hubs/transaction-hub/Dockerfile`.
...
2. **`workflows/deployApplication.yaml`**:
   - Deploys `hubs/transaction-hub/helm` chart across environments (`override_envs`) and regions (`override_regions`).
3. **Renovate & Release-Please Chore PRs**:
   - Creates chore PRs titled `chore(transaction-hub): release`.
   - Automatically bumps `image.tag` in `hubs/transaction-hub/helm/values.yaml` and version in `Chart.yaml`.
