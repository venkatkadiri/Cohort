...
- **Package Location**: `apps/leader-hub`
- **HTTP Port**: `:8007`
...
A multi-stage, rootless production Dockerfile is maintained at `apps/leader-hub/Dockerfile`:
```bash
# Build Docker image
docker build -f apps/leader-hub/Dockerfile -t leader-hub:latest .
...
The cloud infrastructure for this package is self-contained under `apps/leader-hub/terraform/`.
...
Package-level Helm charts are maintained at `apps/leader-hub/helm/`:
...
The package includes dedicated pipelines under `apps/leader-hub/.github/`:
1. **`workflows/releaseApplication.yaml`**:
   - Builds `apps/leader-hub/Dockerfile`.
...
2. **`workflows/deployApplication.yaml`**:
   - Deploys `apps/leader-hub/helm` chart across environments (`override_envs`) and regions (`override_regions`).
3. **Renovate & Release-Please Chore PRs**:
   - Creates chore PRs titled `chore(leader-hub): release`.
   - Automatically bumps `image.tag` in `apps/leader-hub/helm/values.yaml` and version in `Chart.yaml`.
