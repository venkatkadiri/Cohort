...
Credits and economics manager for credit allocation, ledger accounting, and subscription billing.

---

## 📌 Service Specifications
- **Package Location**: `apps/transaction-hub`
- **HTTP Port**: `:8008`
- **Health Check**: `http://localhost:8008/health`
- **Prometheus Metrics**: `http://localhost:8008/metrics`
...
pnpm --filter @cohort/transaction-hub dev

# Typecheck TypeScript code
pnpm --filter @cohort/transaction-hub typecheck

# Build production bundle
pnpm --filter @cohort/transaction-hub build
```

### 2. Docker Containerization
A multi-stage, rootless production Dockerfile is maintained at `apps/transaction-hub/Dockerfile`:
```bash
# Build Docker image
docker build -f apps/transaction-hub/Dockerfile -t transaction-hub:latest .

# Run container locally
docker run -p 8008:8008 transaction-hub:latest
```
...
- Feature branches: `feature/transaction-hub/<short-description>` (e.g. `feature/transaction-hub/add-filter-options`)
- Bugfix branches: `fix/transaction-hub/<short-description>` (e.g. `fix/transaction-hub/handle-null-pointer`)
- Chore branches: `chore/transaction-hub/<short-description>`
...
- **Scope**: Must match this package name: `(transaction-hub)`
- **Examples**:
  - `feat(transaction-hub): add webhook notification dispatcher`
  - `fix(transaction-hub): correct database connection retry logic`
  - `refactor(transaction-hub): migrate configuration parser to strict schema`

### 3. Local PR Title Validation
Verify your proposed PR title locally before submitting:
```bash
pnpm lint:pr "feat(transaction-hub): support real-time updates"
```
...
The cloud infrastructure for this package is self-contained under `apps/transaction-hub/terraform/`.
...
./cohort-tools.sh tf transaction-hub validate
# or validate across all packages:
./cohort-tools.sh tf:validate:all

# Run Terraform plan (dry-run)
./cohort-tools.sh tf transaction-hub plan
```
...
Package-level Helm charts are maintained at `apps/transaction-hub/helm/`:
...
./scripts/minikube.sh build-images transaction-hub
...
The package includes dedicated pipelines under `apps/transaction-hub/.github/`:
1. **`workflows/releaseApplication.yaml`**:
   - Builds `apps/transaction-hub/Dockerfile`.
   - Pushes image to `ghcr.io/${{ github.repository_owner }}/transaction-hub` (without JFrog).
   - Tags image with commit SHA, `:latest`, and semantic version.
2. **`workflows/deployApplication.yaml`**:
   - Deploys `apps/transaction-hub/helm` chart across environments (`override_envs`) and regions (`override_regions`).
3. **Renovate & Release-Please Chore PRs**:
   - Creates chore PRs titled `chore(transaction-hub): release`.
   - Automatically bumps `image.tag` in `apps/transaction-hub/helm/values.yaml` and version in `Chart.yaml`.
